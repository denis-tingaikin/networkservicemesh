...
func SetupNodesConfig(k8s *K8s, nodesCount int, timeout time.Duration, conf []*pods.NSMgrPodConfig, namespace string) ([]*NodeConf, error) {
	...
	for ii := 0; ii < nodesCount; ii++ {
		...
		go func() {
			...
			if err != nil {
				logrus.Errorf("Failed to OnArtifactsSave NSMgr/Forwarder: %v on node %s %v", time.Since(startTime), node.Name, err)
				resultError = err
				return
			}
			if debug {
			...
			logrus.Printf("OnArtifactsSave NSMgr/Forwarder: %v on node %s", time.Since(startTime), node.Name)
			confs[i] = &NodeConf{
				Nsmd:      nsmd,
				Forwarder: forwarder,
				Node:      &nodes[i],
...
func deployProxyNSMgr(k8s *K8s, template *v1.Pod, node *v1.Node, timeout time.Duration) (pnsmd *v1.Pod, err error) {
	...
	if err != nil {
		logrus.Errorf("Failed to OnArtifactsSave pNSMgr: %v on node %s %v", time.Since(startTime), node.Name, err)
		return nil, err
	}
